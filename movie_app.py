...
try:
    # We use ttl=0 to skip the cache entirely
    df = conn.read(spreadsheet=SHEET_URL, worksheet=ws_name, ttl=0)
    
    # Success! Process the data
    df.columns = df.columns.str.strip()
    item_col = df.columns[0] # Use the first column as the title regardless of name
    ...
    st.error(f"⚠️ Error: Could not find the tab named '{ws_name}'")
    st.info("Please follow the Hard Reset steps below.")
    st.code(e)
